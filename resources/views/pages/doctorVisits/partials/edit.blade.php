            ...
            <div class="col-12">
                <div class="card shadow-sm border-0">
                    ...
                    <div class="card-body p-4 p-md-5">
                        <x-forms.form :action="route('doctorVisits.update', $doctorVisit)" method="POST" enctype="multipart/form-data" class="needs-validation"
                            ...
                            <div class="row mb-4">
                                <div class="col-12">
                                    <x-forms.choices-select name="doctor_id" label="{{ __('Doctor') }}" :options="$data['doctorOptions'] ?? []" :model="$doctorVisit"
                            ...
                            <div class="mb-5">
                                ...
                                @php
                                    ...
                                    $rows = [];
                                    if (is_array($oldProductIds) && count($oldProductIds) > 0) {
                                        foreach ($oldProductIds as $idx => $pid) {
                                            $rows[] = [
                                                'product_id' => $pid,
                                                'quantity' => $oldQuantities[$idx] ?? 1,
                                            ];
                                        }
                                    } else {
                                        $samples = $doctorVisit->samples ?? collect();
                                        if ($samples->count()) {
                                            ...
                                            $rows[] = ['product_id' => null, 'quantity' => 1];
                                        }
                                    }
                                @endphp
                                <div>
                                                ...
                                                <tr class="visit-item">
                                                    <td class="p-3">
                                                        <x-forms.choices-select name="product_id[]" :options="$data['productOptions'] ?? []"
                                                            :value="$row['product_id']" placeholder="{{ __('Select Product') }}"
                                                    ...
                                                    <td class="p-3">
                                                        <x-forms.input name="quantity[]" type="number"
                                                            value="{{ $row['quantity'] }}" min="1" required
                                                            col="12" />
                            ...
                            <div class="row g-3 mb-4">
                                <div class="col-12 col-md-6">
                                    <x-forms.input name="visit_date" type="date" label="{{ __('Visit Date') }}"
                                        value="{{ old('visit_date', optional($doctorVisit->visit_date)->format('Y-m-d')) }}" required col="12" />
                                </div>
                                <div class="col-12 col-md-6">
                                    <div class="mb-3">
                                        ...
                                        @enderror
                                        <div id="attachment_file_name" class="form-text text-muted mt-2">
                                            @if ($doctorVisit->image)
                                                {{ basename($doctorVisit->image) }}
                                            @endif
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="mb-4">
                                <x-forms.textarea name="note" label="{{ __('Note') }}" rows="4"
                                    :value="old('note', $doctorVisit->notes)" placeholder="{{ __('Enter any additional notes...') }}" />
                            </div>
