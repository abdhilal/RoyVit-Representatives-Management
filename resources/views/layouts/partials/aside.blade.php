<aside class="page-sidebar {{ session('icon') === 'Colorful' ? 'iconcolor-sidebar' : '' }}"
    data-sidebar-layout="{{ session('icon') === 'Colorful' ? 'iconcolor-sidebar' : 'stroke-svg' }}">
        ...
        <ul class="sidebar-menu" id="simple-bar">
                ...
                @foreach ($section['items'] ?? [] as $item)
                    @php
                        ...
                        $children = array_values(
                            array_filter($children, function ($c) use ($user) {
                                ...
                                <ul class="sidebar-submenu"
                                    ...
                                    @foreach ($children as $child)
                                        ...
                                        <li class="{{ $childActive ? 'active' : '' }}">
                                            <a href="{{ $childHref }}"
                                                @if (!empty($child['color'])) style="color: {{ $child['color'] }}" @endif>{{ __($child['label'] ?? '') }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            @endif
            ...
            <div class="p-3" style="position: sticky; bottom: 0; background: var(--bs-body-bg);">
                <form action="{{ route('logout') }}" method="POST" class="w-100">
                    @csrf
                    <a href="javascript:void(0)" class="sidebar-link text-danger d-flex align-items-center gap-2"
